...
from .utils.ssd_box_utils import center_size
...
def inference(cfg, net, img_path):
    ...
    if landm is not None:
        landms = decode_landm(landm.data.squeeze(0), prior_data,
                              cfg['variance'])
        print('decode_landm', landms)
        scale_landm = torch.Tensor([
            width, height, width, height, width, height, width, height, width,
            height
        ])
        ...
        dets = np.concatenate((dets, landms),
                              axis=1)  # concate boxes and landms
