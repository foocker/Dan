...
from dan.detection.core.center.image import flip, color_aug
from dan.detection.core.center.image import get_affine_transform, affine_transform
from dan.detection.core.center.image import gaussian_radius, draw_umich_gaussian, draw_msra_gaussian
from dan.detection.core.center.image import draw_dense_reg
import math
    ...
    def __getitem__(self, index):
        ...
        if self.split == 'train':
            if not self.opt.not_rand_crop:
                ...
                )  # does not crop the img, only imagine the center locat
        ...
        wh = np.zeros((self.max_objs, 2),
                      dtype=np.float32)  # max_0bjs can reduce from label.json
        ...
        for k in range(num_objs):
            ...
            bbox[:2] = affine_transform(
                bbox[:2], trans_output)  # affine on bbox to heatmap scale
            ...
            if h > 0 and w > 0:
                radius = gaussian_radius(
                    (math.ceil(h), math.ceil(w)))  # up integer
                ...
                draw_gaussian(hm[cls_id], ct_int,
                              radius)  # hm[cls_id].shape, (128, 128)
                ...
                ind[k] = ct_int[1] * output_w + ct_int[
                    0]  # y_c * output_w + x_c:index of max heat on heatmap img(128, 128)
                ...
                gt_det.append([
                    ...
                    1, cls_id
                ])  # xyxy, 1, cls_id on heatmap, not org img
