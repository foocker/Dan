...
def resize_keep(img, max_length):
    h, w, _ = img.shape
    if h > w:
        img = cv.resize(img, (int(max_length * w / h), int(max_length)),
                        interpolation=cv.INTER_LINEAR)  # (w, h)
    else:
        img = cv.resize(img, (int(max_length), int(max_length * h / w)),
...
def gradcutmat(temp, bg, mode='template', ration=0.3):
    ...
    if mode == 'detect':
        # model detect result, and crop, mask will be better
        # src =
        pass
    h_s, w_s, _ = src.shape
    ...
    result = a * (bgback.astype(np.float32)) + (1 - a) * (background.astype(
        np.float32))
        ...
        for img_name in os.listdir(os.path.join(temp_resources, cla_name)):
            ...
            for fimg in dsts:
                try:
                    ...
                    fuseimg = gradcutmat(os.path.join(temp_resources, cla_name,
                                                      img_name),
                                         fimg,
                                         ration=ration)
                    fusedname = img_name[:-4] + '_' + os.path.basename(fimg)
                    cv.imwrite(
                        os.path.join(temp_resources, cla_name, fusedname),
                        fuseimg)
                except:
                    print(fusedname)
                    continue
    ...
    for cla_name_dir in temp_resources_list:
        for img_name in os.listdir(cla_name_dir):
            ...
            for fimg in dsts:
                try:
                    ration = random.choice([0.3, 0.35, 0.4, 0.42])
                    fuseimg = gradcutmat(os.path.join(cla_name_dir, img_name),
                                         fimg,
                                         ration=ration)
                    fusedname = img_name[:-4] + '_' + os.path.basename(fimg)
...
def muli_pthread(num_process, bg_resources, temp_resources):
    ...
    for i in range(num_process):
        print('run process:', i)
        p.apply_async(creat_cig_data_,
                      args=(bg_resources,
                            temp_resources[part_list[i]:part_list[i + 1]]))
