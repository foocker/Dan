...
def confusion_matrix_eval(data_loader, model, n_classes, mode='onelabel'):
    ...
    with torch.no_grad():
        for imgs, labels, paths in data_loader:
            ...
            label_mul_single = torch.zeros(
                labels.shape[0])  # transform one-hot label to its index
            ...
            if mode == 'onelabel':
                ...
                for vec_lab, vec_pred in zip(labels, probality):
                    ...
                    temp = torch.tensor([
                        1 if i in indx else 0 for i in range(n_classes)
                    ])  # contain __ignore__
            ...
            for i in range(labels.shape[0]):
                results[paths[i].split('/')[-1]] = [
                    classes[preds[i].item()],
                    '{:.3f}'.format(probality[0, preds[i].item()].item())
                ]
            conf_matrix_path = confusion_matrix_batch_path(
                preds, labels, paths, conf_matrix_path)
...
def handel_conf_matrix_path():
    ...
    for k, vs in data.items():
        ...
        for v in vs:
            shutil.copyfile(v, temp_data_path + '/' + k + '/' +
                            v.split('/')[-1])  # linux
...
class DynamicAdjustData(object):
    ...
    def rename_imgs(self, imagepath, extra_info):
        ...
        for cla_name in os.listdir(imagepath):
            for i, file_name in enumerate(
                    os.listdir(os.path.join(imagepath, cla_name))):
                ...
                dst = os.path.join(
                    imagepath, cla_name,
                    cla_name + '_' + str(i) + extra_info + '_.' + f_format)
                os.rename(scr, dst)
    ...
    def resize_imgs(self, imagepath, resized_shape):
        ...
        for cla_name in os.listdir(imagepath):
            for file_name in os.listdir(os.path.join(imagepath, cla_name)):
                ...
                if f_format not in self.file_format:
                    continue
                img = cv2.imread(os.path.join(imagepath, cla_name, file_name),
                                 cv2.IMREAD_COLOR)
                resized_img = cv2.resize(img,
                                         resized_shape,
                                         interpolation=cv2.INTER_CUBIC)
                cv2.imwrite(os.path.join(imagepath, cla_name, file_name),
                            resized_img)
    ...
    def move_checked_ml(self, claname, checked_p, dest):
        ...
        for img_f in os.listdir(checked_p):
            try:
                os.rename(join(data_dir_test, claname, img_f),
                          join(dest, img_f))
                os.remove(join(checked_p, img_f))
            except:
    ...
    def move_checkedimg_test_train(self, claname, checked_p, ratio=0.8):
        ...
        for img_f in os.listdir(checked_p)[:move_num]:
            try:
                os.rename(join(data_dir_test, claname, img_f),
                          join(data_dir, claname, img_f))  # or replace
                os.remove(join(checked_p, img_f))
            except:
                print('move_checkedimg_test_train', img_f)
                continue
        print(
            'move imgs from test to train which names in {} at ratio {} done!'.
            format(checked_p, ratio))
