from .models.primary_resnet import create_model, get_params_to_update, reusemodel, progressive_model
...
from .apis.trainer import train_classify_fun
...
criterion = nn.CrossEntropyLoss(
    weight=label_w
) if cfg.datalabelmode == 'onelabel' else nn.BCEWithLogitsLoss()
...
def train(mode='train_primary'):
    assert mode in [
        'train_primary', 'pri_finetune', 'pro_finetune', 'progressive'
    ]
    ...
    for i in range(len(datasets)):
        ...
        if mode == 'train_primary':
            ...
            model, best_epoch_before = progressive_model(
                model, cfg.weight_path,
                cfg.num_class)  # not new_class for evaluate create model
        else:
        ...
        if cfg.lr_scheduler:
            scheduler = MultiStepLR(op,
                                    milestones=cfg.milestones,
                                    gamma=cfg.lr_gamma)
        else:
            scheduler = None

        ohist = train_classify_fun(cfg.datainfo,
                                   labelmode,
                                   cfg.multi_threshold,
                                   cfg.save_path,
                                   model,
                                   ...
                                   op,
                                   scheduler=scheduler,
                                   best_epoch_before=best_epoch_before,
                                   epoch_plus=cfg.epoch_plus,
